 ...
 */
package main.application.forms.masterconsumption;
...
import main.model.table.ConsumptionTableModel;
import main.service.ConsumptionService;
 ...
 * @author aldes
 */
public class MasterConsumptionForm extends JPanel {

    private ConsumptionService service = new ConsumptionService();
    private ConsumptionTableModel model;
    ...
    private void init() {
        ...
        header.setTitleText("Master Konsumsi");
    ...
    private void initTable(JPanel panel) {
        model = new ConsumptionTableModel();
    ...
    private void populateForm(Consumption consumption) {
        clearForm();

        Long id = consumption.getId();

        if (id != null) {
            groupId.getInputField().setText(String.valueOf(consumption.getId()));
        }

        groupName.getInputField().setText(consumption.getName());
    }
    ...
    private void edit(int row) {
        Consumption data = (Consumption) model.find(row);
        populateForm(data);
        atTableRowIndex = row;
    }
